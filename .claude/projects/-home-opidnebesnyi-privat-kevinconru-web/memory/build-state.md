...
- **Old-site (Format) content fully imported 2026-06-04** — see `planning/import-plan.md`. Pipeline: `scripts/generate-format-manifest.py` (parses the crawled Format pages, pairs each gallery image with its DOM caption, matches local `media/` originals by NFC filename, signed CDN URL fallback) → `scripts/format-manifest.json` → `npm run import:format` (sharp ≤2400px JPEG, GCS skip-if-exists, upsert by slug, gallery delete+recreate, sortOrder = year desc/title asc). Result: 22 publications / 13 press / 144 gallery images. Verify-in-admin: Bernatzik years 2003-05, Tribalmania 2005, TAS 2020 (best-effort). `SoutheastAsia` Region enum added (migration `add_southeast_asia_region`); it has no filter pill (shows under "All").
